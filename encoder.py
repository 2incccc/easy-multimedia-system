...
def write_to_file(filepath, dc, ac, blocks_count, tables):
    ...
    for table_name in ['dc_y', 'ac_y', 'dc_c', 'ac_c']:
        ...
        f.write(uint_to_binstr(len(tables[table_name]), 16).encode())
        ...
        for key, value in tables[table_name].items():
            if table_name in {'dc_y', 'dc_c'}:
                ...
                f.write(uint_to_binstr(key, 4).encode())
                f.write(uint_to_binstr(len(value), 4).encode())
                ...
                # 'code_length' bits for 'huffman_code'
                f.write(uint_to_binstr(key[0], 4).encode())
                f.write(uint_to_binstr(key[1], 4).encode())
                f.write(uint_to_binstr(len(value), 8).encode())
                f.write(value.encode())

    # 32 bits for 'blocks_count'
    f.write(uint_to_binstr(blocks_count, 32).encode())
    ...
    for b in range(blocks_count):
        for c in range(3):
            ...
            f.write(dc_table[category].encode())
            f.write(int_to_binstr(dc[b, c]).encode())

            for i in range(len(symbols)):
                f.write(ac_table[tuple(symbols[i])].encode())
                f.write(values[i].encode())
    f.close()
